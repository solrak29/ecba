...
function saveUser(id, name, from, callback) {
    var sql;
    if ( from == "FB" ) {
        sql = "insert into mycoop_users ( first_name, facebook_id ) values ( \'"+ name + "\'," + id + ")";
        console.log("Sql being used: " + sql);
    } else {
        console.log("Uknown sender not saving any data");
    }
    connection.query(sql, function( err, result ) {
        if (err) {
           callback( "ERROR in saving user" + err, null);
        } else {
        ...
        if (err) {
           ...
           callback( "ERROR in getting all users", null);
	        ...
	        var parsedjson = parseJson(JSON.stringify(rows));
                console.log(" Returned from db : " + parsedjson);
